...
if len(sys.argv) < 2:
    print("Usage: python generate_agent_prompt.py <chunk_index>")
    sys.exit(1)

chunk_index = int(sys.argv[1])

with open("graphify-out/chunks_info.json", "r") as f:
...
# Check if --mode deep was given (assume not for now, but we can pass it if needed)
...
Files (chunk {chunk_index + 1} of {total_chunks}):
{file_list_str}
