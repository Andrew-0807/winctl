...
- **Mobile UX improvements** — Slide-in animation, full-screen width on small phones (≤480px), larger and centered nav text.

## Capabilities

### New Capabilities

- `sidebar-overhaul`: Delivers a clean sidebar with no duplicate controls, correct mobile close behaviour, and a smooth slide-in animation.

### Modified Capabilities

- `Sidebar.tsx`: Views section removed; import list trimmed.
- `global.css`: Mobile breakpoint updated with animation, full-screen width, hidden collapse button, and larger centered text.

## Impact

- **SolidJS/src/components/Sidebar.tsx** — Views section and related code removed.
- **SolidJS/src/styles/global.css** — Mobile media query updated.
- No server changes. No store changes.
